...
(function () {
  ...
  function buildMachine() {
    // 主板（绿色 PCB 底板）
    const deck = makeDeck(820, 520, 'deck-mobo', 'MOTHERBOARD · 点击部件逐层深入');
    place(deck, 0, 0.15, 0, -Math.PI / 2);
    groupMachine.add(deck);

    // CPU（左中，散热器样式）
    const cpu = makeCard('m-cpu', 150, 70, { name: 'CPU', sub: '内核 · 调度 · MMU', cls: 'c3d-kernel' });
    place(cpu, -3.6, 0.8, 1.1);
    groupMachine.add(cpu);

    // 内存条 ×4（中排横列）
    for (let i = 0; i < 4; i++) {
      const ram = makeCard('m-ram', 44, 130, { name: '', sub: '', cls: 'c3d-term' });
      place(ram, -0.3 + i * 0.95, 0.8, 1.9, 0, 0, Math.PI / 2);
      groupMachine.add(ram);
    }
    const lRam = makeCard('m-ram', 120, 30, { name: 'RAM ×4', sub: '虚拟内存 · 页表 · 缺页', cls: 'c3d-sub' });
    place(lRam, 0.95, 0.35, 3.6);
    groupMachine.add(lRam);

    // 硬盘（右中）
    const disk = makeCard('m-disk', 160, 60, { name: 'SSD', sub: '目录树 · VFS · ext4', cls: 'c3d-metal' });
    place(disk, 3.6, 0.8, 1.2);
    groupMachine.add(disk);

    // 电源（右后）
    const psu = makeCard('m-psu', 120, 60, { name: 'PSU', sub: '上电 → 引导 → start_kernel', cls: 'c3d-metal' });
    place(psu, 4.6, 0.8, -2.4);
    groupMachine.add(psu);

    // 网卡（左下）
    const nic = makeCard('m-net', 110, 45, { name: 'NET', sub: '协议栈 · 中断', cls: 'c3d-gate' });
    place(nic, -5.0, 0.75, -1.8);
    groupMachine.add(nic);

    // 屏幕（后侧立起）
    const screen = makeCard('m-screen', 180, 100, { name: 'SCREEN', sub: 'libc · 系统调用 · 进程', cls: 'c3d-user' });
    place(screen, 0, 2.3, -3.6);
